...
/**
 * Generate SOUL.md content from agent.soul config and auto-detected stack.
 * Returns null if no soul config is defined (soul is opt-in).
 */
export function generateSoulMarkdown(
  config: BootConfig,
  cwd: string
): string | null {
  const soul = config.agent?.soul;
  if (!soul) return null;
  ...
  if (soul.voice && soul.voice.length > 0) {
    lines.push("## Voice");
    lines.push("");
    for (const v of soul.voice) {
      lines.push(`- ${v}`);
    }
    lines.push("");
  }

  // Team soul (merged separately so it's labeled)
  const teamConfig = getTeamConfigSeparately(cwd);
  const teamSoul = teamConfig?.agent?.soul;
  if (teamSoul) {
    const teamValues = teamSoul.values || [];
    const teamBoundaries = teamSoul.boundaries || [];
    const teamVoice = teamSoul.voice || [];

    const projectValues = new Set(soul.values || []);
    const projectBoundaries = new Set(soul.boundaries || []);
    const projectVoice = new Set(soul.voice || []);

    const uniqueTeamValues = teamValues.filter((v) => !projectValues.has(v));
    const uniqueTeamBoundaries = teamBoundaries.filter((b) => !projectBoundaries.has(b));
    const uniqueTeamVoice = teamVoice.filter((v) => !projectVoice.has(v));

    if (uniqueTeamValues.length > 0 || uniqueTeamBoundaries.length > 0 || uniqueTeamVoice.length > 0) {
      lines.push("## Team");
      lines.push("");
      if (uniqueTeamValues.length > 0) {
        for (const v of uniqueTeamValues) {
          lines.push(`- ${v}`);
        }
      }
      if (uniqueTeamBoundaries.length > 0) {
        for (const b of uniqueTeamBoundaries) {
          lines.push(`- ${b}`);
        }
      }
      if (uniqueTeamVoice.length > 0) {
        for (const v of uniqueTeamVoice) {
          lines.push(`- ${v}`);
        }
      }
      lines.push("");
  ...
  if (stack.length > 0) {
    lines.push("## Context");
    lines.push("");
    lines.push(`- **Stack**: ${stack.map(formatStackName).join(", ")}`);
    lines.push(`- **Package manager**: ${pm}`);
    lines.push("");
  }
...
/**
 * Generate SKILL.md content from auto-detected skills and user-defined skills.
 * Always generates output (auto-skills provide value with zero config).
 */
export function generateSkillMarkdown(
  ...
  // Merge: user skills override auto-generated ones with the same name
  ...
  if (mergedSkills.length === 0) return null;
 ...
 * Also writes SOUL.md and SKILL.md when content is provided.
...
export function syncTargets(
  ...
  markdown: string,
  cwd: string,
  options: { overwrite?: boolean; soulMarkdown?: string | null; skillMarkdown?: string | null } = {}
): SyncTargetsResult {
  ...
  // SOUL.md
  if (options.soulMarkdown) {
    const soulPath = path.join(cwd, "SOUL.md");
    if (!fs.existsSync(soulPath) || overwrite) {
      fs.writeFileSync(soulPath, options.soulMarkdown);
      written.push("SOUL.md");
    } else {
      skipped.push("SOUL.md");
    }
  }

  // SKILL.md
  if (options.skillMarkdown) {
    const skillPath = path.join(cwd, "SKILL.md");
    if (!fs.existsSync(skillPath) || overwrite) {
      fs.writeFileSync(skillPath, options.skillMarkdown);
...
export function checkSync(
  ...
  // Check SKILL.md
  const skillMd = generateSkillMarkdown(config, cwd);
  if (skillMd) {
    checkFile(path.join(cwd, "SKILL.md"), skillMd, "SKILL.md", result);
  }
